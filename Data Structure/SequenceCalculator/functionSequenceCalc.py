...
def calculate_arithmetic_sum(first_term, common_difference, n):
    return n * (2 * first_term + (n - 1) * common_difference) / 2


# Function to calculate the sum of a geometric sequence
def calculate_geometric_sum(first_term, common_ratio, n):
    if common_ratio == 1:
        return n * first_term
    else:
...
def main():
    ...
    if sequence_type == 1:
        ...
        result = calculate_arithmetic_sum(first_term, common_difference, n)
        ...
        n = int(input("Enter the number of terms (n): "))
        result = calculate_geometric_sum(first_term, common_ratio, n)
